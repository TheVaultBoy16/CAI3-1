...
def register_user(email: str, password: str):
    ...
    if response.status_code == 200:
        ...
        img.show()
        print("📱 Escanea este QR con Google Authenticator o Authy.")
        print("Luego usa tu app para generar el código OTP de 6 dígitos.")
    else:
        print("⚠️ Error al registrar:", response.text)
...
def verify_otp(email: str):
    ...
    if not code.isdigit() or len(code) != 6:
        print("⚠️ Código inválido. Debe ser un número de 6 dígitos.")
        return
...
def main():
    print("\n=== 🧠 Cliente 2FA ===")
    print("1️⃣ Registrar nuevo usuario")
    print("2️⃣ Iniciar sesión con usuario existente")
